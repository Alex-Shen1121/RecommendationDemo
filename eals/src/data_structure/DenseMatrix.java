...
public class DenseMatrix implements Serializable {
	 ...
	 */
	public data_structure.DenseVector row(int rowId) {
	 ...
	 */
	public data_structure.DenseVector row(int rowId, boolean deep) {
		return new data_structure.DenseVector(data[rowId], deep);
	}
	 ...
	 */
	public data_structure.DenseVector column(int column) {
		data_structure.DenseVector vec = new data_structure.DenseVector(numRows);
	 ...
	 */
	public data_structure.DenseVector mult(data_structure.DenseVector vec) {
		assert this.numColumns == vec.size;

		data_structure.DenseVector res = new data_structure.DenseVector(this.numRows);
	...
	public DenseMatrix cov() {
		...
		for (int i = 0; i < numColumns; i++) {
			data_structure.DenseVector xi = this.column(i);
			xi = xi.minus(xi.mean());

			mat.set(i, i, xi.inner(xi) / (xi.size - 1));

			for (int j = i + 1; j < numColumns; j++) {
				data_structure.DenseVector yi = this.column(j);
	 ...
	 */
	public void setRow(int row, data_structure.DenseVector vals) {
		for (int j = 0; j < numColumns; j++)
